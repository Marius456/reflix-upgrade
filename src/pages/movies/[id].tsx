...
import { addNewDocument, isDocExist, removeDocument } from "../../context/firebase";
...
export default function MovieDatailsPage({ movie }: { movie: IMovie }) {
    ...
    return (
        <>
            ...
            <div className={styles.container_moviedetails}>
                ...
                <div className={styles.movie_info}>
                    ...
                    <>
                        {data && (
                            IsInLibrary ? (
                                <div id="unfavorite_movie" className={styles.modal_btn} onClick={() => removeDocument(movie?.title || "")}>
                                    <p>Remove from Library</p>
                                </div>
                            ) : (
                                <div id="favorite_movie" className={styles.modal_btn} onClick={() => addNewDocument(movie?.title || "")}>
                                    <p>Add to Library</p>
                                </div>
                            ))
                        }
            ...
            {movie &&
                <MoviesCategory category={movie.genres[0]} />
            }
            <MovieTrailer
                isOpen={modalShow}
